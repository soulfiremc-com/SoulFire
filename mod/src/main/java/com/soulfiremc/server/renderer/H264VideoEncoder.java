...
@Slf4j
public final class H264VideoEncoder implements AutoCloseable {
  ...
  H264VideoEncoder(int width, int height, String requested) {
  ...
  private static final class NativeEncoder implements AutoCloseable {
    ...
    private NativeEncoder(String name, int width, int height) {
      ...
      try {
        ...
        context.width(width).height(height).pix_fmt(pixelFormat).max_b_frames(0).gop_size(FPS)
          ...
          .flags(AV_CODEC_FLAG_LOW_DELAY | AV_CODEC_FLAG_CLOSED_GOP).profile(AV_PROFILE_H264_BASELINE).level(42)
        ...
        try {
          switch (name) {
            case "libx264" -> {
              option(options, "preset", "ultrafast");
        ...
        input = new BytePointer((long) width * height * 4);
        inputPlanes = new PointerPointer<>(1);
        inputPlanes.put(0, input.getPointer((long) width * (height - 1) * 4));
    ...
    private EncodedFrame encode(byte[] rgba, long timestampUs, boolean requestedKeyFrame) {
      ...
      try {
        ...
        if (key) codecString = codecFromSps(bytes);
